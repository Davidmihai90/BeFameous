...
export default function InfluencerAltele() {
  const { user, profile } = useAuth();
  const [packs, setPacks] = useState(profile?.otherPackages || []);
  const [saving, setSaving] = useState(false);

  const addPack = () => setPacks([...packs, { platforma: '', tip: '', pret: '', descriere: '' }]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updateDoc(doc(db, 'users', user.uid), { otherPackages: packs });
      alert('Pachetele pentru alte platforme au fost salvate!');
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
      <h2 className="text-xl font-semibold mb-2">Alte platforme — pachete & prețuri</h2>
      <p className="text-white/60 mb-4 text-sm">
        Ex: YouTube (UGC), Twitch, Snapchat, Pinterest, Blog etc.
      </p>
        ...
        {packs.map((p, i) => (
          <div key={i} className="border border-white/10 rounded-xl p-4">
            <div className="grid md:grid-cols-3 gap-3">
              <input
                placeholder="Platforma (ex: YouTube)"
                value={p.platforma || ''}
                onChange={(e) => updateField(i, 'platforma', e.target.value)}
