...
export default function InfluencerTiktok() {
  const { user, profile } = useAuth();
  const [packs, setPacks] = useState(profile?.tiktokPackages || []);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updateDoc(doc(db, 'users', user.uid), { tiktokPackages: packs });
      alert('Pachetele TikTok au fost salvate!');
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
      <h2 className="text-xl font-semibold mb-2">TikTok — pachete & prețuri</h2>
      <p className="text-white/60 mb-4 text-sm">
        Adaugă pachete pentru video, duo, live, serii etc.
      </p>
